...
function Get-KolektriaHotFixKbs {
    <#
    .SYNOPSIS
    ...
    try {
        return @(
            Get-HotFix |
                ForEach-Object { ConvertTo-KolektriaKbId -Value $_.HotFixID } |
                Where-Object { $_ } |
                Sort-Object -Unique
        )
    }
...
function Get-KolektriaPackageKbs {
    <#
    .SYNOPSIS
    ...
    try {
        return @(
            Get-WindowsPackage -Online |
                ForEach-Object {
                    $packageKb = ConvertTo-KolektriaKbId -Value $_.PackageName
                    ...
                    ConvertTo-KolektriaKbId -Value $_.Description
                } |
                Where-Object { $_ } |
                Sort-Object -Unique
        )
...
function Get-KolektriaInventory {
    <#
    .SYNOPSIS
        Build the Kolektria installed update inventory object.
    #>

    $isAdmin = Test-KolektriaAdministrator

    $hotFixKbs = @(Get-KolektriaHotFixKbs)
    $packageKbs = @(Get-KolektriaPackageKbs -IsAdmin $isAdmin)
...
if ($MyInvocation.InvocationName -ne '.') {
    Get-KolektriaInventory | ConvertTo-Json -Depth 4
}